...
- Add auth to a delete endpoint and check that works on remote with strict cookie settings
- Tidy up unit tests for new auth stuff
- Tidy up env var for jwt secret keys and make sure the secret has enough entropy
- Decompose filtering logic after unit tests are complete
- Come up with more sensible config driven way of handling cookies during local development
...
- Add new endpoint for filtering that includes private records
